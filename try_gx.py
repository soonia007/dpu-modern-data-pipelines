...
def _validate_data():
    ...
    my_df = pd.read_csv("data.csv", names=columns) #read csv by within column
    print(my_df)

    valid_df = my_df[(my_df.NewConfirmed >= 0 ) & (my_df.NewConfirmed <= 322315)]
    dq = len(valid_df) / len(my_df) 
